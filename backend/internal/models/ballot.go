...
func (b *Ballot) GetUserID() string {
	return b.userID
}

// ValidateFields ensures that all fields are non-empty, the rank order has at least two rankings,
...
// UnmarshalJSON is custom unmarshaler that handles new ballot creation when the data comes
// directly from JSON.
func (b *Ballot) UnmarshalJSON(data []byte) error {
	// Create an auxiliary struct to unmarshal the fields
	aux := &struct {
